...
function App() {
  ...
  const addItem = () => {
    ...
    const newItem = {
      ...
      dsc: parseFloat(dscRef.current.value)
    };

    const existingIndex = dataItems.findIndex(
      (v) => v.item === newItem.item && v.ppu === newItem.ppu && v.dsc === newItem.dsc
    );

    if (existingIndex >= 0) {
      let updatedDataItems = [...dataItems];
      updatedDataItems[existingIndex].qty += newItem.qty;
        ...
        <Col md={4} style={{ backgroundColor: "#e4e4e4" }}>
          ...
          <Row>
            <Col>
                ...
                <Form.Control placeholder="Type discount" type="number" ref={dscRef} defaultValue={10}/>
            </Col>
          </Row>
          <hr />
